...
namespace GMPPKConverter.Tests
{
    public class Test_DSA
    {
        ...
        [Theory]
        [MemberData(nameof(TestData.DSA_Data_OpenSSH), MemberType = typeof(TestData))]
        public void Test_DSA_OpenSSH(string keyData, string inPassword, byte[][] RngData, string Result)
        {
        ...
        [Theory]
        [MemberData(nameof(TestData.DSA_Data_PEM), MemberType = typeof(TestData))]
        public void Test_DSA_PEM(string keyData, string inPassword, string Result)
        {
            System.Text.Encoding.RegisterProvider(System.Text.CodePagesEncodingProvider.Instance);
            // arrange
            string[] key = keyData.TrimStart().Split("\r\n");
